...
api.get("/user-items", async (c) => {
  ...
  const result = await database().prepare(history ? "SELECT content_id AS contentId, progress, updated_at AS date FROM watch_history WHERE user_email = ? ORDER BY updated_at DESC" : "SELECT content_id AS contentId, created_at AS date FROM library_items WHERE user_email = ? ORDER BY id DESC").bind(user.email).all();
  return c.json({ items: result.results });
});

api.post("/library", async (c) => { const user = await ensureUser(c.req.raw); const { contentId } = await c.req.json<{ contentId: string }>(); await database().prepare("INSERT OR IGNORE INTO library_items (user_email, content_id) VALUES (?, ?)").bind(user.email, contentId).run(); return c.json({ ok: true }); });
