...
export function CollectionPage({ kind }: { kind: "history" | "library" }) {
  ...
  const items = records.map((record) => ({ record, item: allItems.find((entry) => entry.id === record.contentId) })).filter((entry): entry is { record: typeof records[number]; item: CatalogItem } => Boolean(entry.item));
