...
function CatalogCard({item}:{item:CatalogItem}){const total=item.type==="anime"?item.episodes:item.chapters;const count=`${typeof total==="number"&&total>0?total:"?"} ${item.type==="anime"?"АНГИ":"БҮЛЭГ"}`;return <Link className="catalog-card" href={`/title/${item.id}`} prefetch={false} aria-label={`${item.title} дэлгэрэнгүй`}><div className="catalog-poster"><img src={item.image} alt={`${item.title} нүүр зураг`} loading="lazy"/><small className="catalog-count">{count}</small></div><div><h2>{item.title}</h2></div></Link>}
