...
export default async function TitlePage({params}:{params:Promise<{id:string}>}){
  ...
  const primaryHref=isAnime?(firstVideo?`/api/app/media/${encodeURIComponent(firstVideo)}`:"#episodes"):(firstEntry?`/read/${item.id}/${firstEntry.number}`:"#chapters");
  ...
  return <Chrome><main className={`detail-page clean-detail ${isAnime?"anime-detail":"manga-detail"}`}>
    ...
    <div className="detail-columns clean-detail-columns">
      <section className="entries" id={isAnime?"episodes":"chapters"}>
        ...
        {isAnime?<div className="episode-list">{entries.map(episode=>{const key=mediaKeys(episode.mediaKeys)[0];return <a href={key?`/api/app/media/${encodeURIComponent(key)}`:"#episodes"} key={episode.id}><div className="episode-thumb"><img src={item.image} alt=""/><span>EP {episode.number}</span></div><div><b>Анги {episode.number}</b><p>{episode.access==="vip"?"Зөвхөн VIP хэрэглэгч үзнэ":"Бүртгэлтэй хэрэглэгч үзнэ"}</p><small>{key?"▶ Шууд үзэх":"Бичлэг ороогүй"}</small></div></a>})}</div>:<div className="chapter-table">{entries.map(episode=><a href={`/read/${item.id}/${episode.number}`} key={episode.id}><span>{episode.number}</span><b>Бүлэг {episode.number}</b><small>{episode.access==="vip"?"VIP ♔":"Унших →"}</small></a>)}</div>}
      </section>
